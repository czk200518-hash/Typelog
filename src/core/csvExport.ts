...
export function buildCsvExport(global: GlobalStats, files: FileStats[], content: "files" | "all"): string {
  const lines: string[] = [];
  // 区块 A：文件级
  lines.push("# 文件级统计");
  ...
  const dates = sortedDateKeys(global);
  // 区块 B：每日统计
  lines.push("");
  lines.push("# 每日统计");
  lines.push("date,grossTyped,activeMs,peakCpm");
  for (const k of dates) {
    lines.push([k, global.dailyGrossByDate[k] ?? 0, global.dailyActiveByDate[k] ?? 0, global.dailyPeakByDate[k] ?? 0].join(","));
  }
...
export function sortedDateKeys(global: GlobalStats): string[] {
  const set = new Set<string>([
    ...Object.keys(global.dailyActiveByDate),
    ...Object.keys(global.dailyGrossByDate),
    ...Object.keys(global.dailyPeakByDate),
    ...Object.keys(global.heatmap),
  ]);
  return [...set].sort();
}
